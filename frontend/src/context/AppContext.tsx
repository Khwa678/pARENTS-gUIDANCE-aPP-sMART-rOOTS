        ...
        const upgraded = parsed.map((m: any) => {
          ...
          if (lessons.length < 3) {
            while (lessons.length < 3) {
              ...
              lessons.push({
                id: `l${m.week}-${nextId}`,
                title: nextId === 1 ? 'Introduction Video' : nextId === 2 ? 'Deep Dive Training' : 'Weekly Integration Guide',
                duration: '10:00',
                type: 'video',
                completed: false,
                videoUrl: 'https://fast.wistia.net/embed/iframe/eabjoioutk'
              });
            }
          } else if (lessons.length > 3) {
            lessons = lessons.slice(0, 3);
          }
          const finalLessons = lessons.map((l: any, lIndex: number) => {
            const updatedL = { ...l };
            updatedL.type = 'video';
            if (!updatedL.videoUrl || !updatedL.videoUrl.includes('wistia')) {
              updatedL.videoUrl = lIndex % 2 === 0 
                ? 'https://fast.wistia.net/embed/iframe/eabjoioutk' 
                : 'https://fast.wistia.net/embed/iframe/i0iwga8cbj';
            }
            return updatedL;
          });
          return {
